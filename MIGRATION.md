...
- Discovery inventory, dry-run and import now require every planned legacy table and an explicit set of semantic source columns. Missing tables or required columns stop before target writes; dry-run also refuses unreviewed source-only columns or required target-only columns. Inventory entries include their ordered source-column list, so even empty-table schema drift changes the combined fingerprint.
