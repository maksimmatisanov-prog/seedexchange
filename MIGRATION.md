...
- Unit coverage includes a valid exact manifest plus missing-hash, metadata-drift and orphan-file failures. TypeScript, 38 Vitest checks and a clean build passed; the compiled media CLI returned `ready: true` against an empty migrated PostgreSQL 14 database and an existing empty isolated media root.
...
- An isolated PostgreSQL 14 run with one generated WebP and a NULL database hash confirmed zero dry-run writes, one explicit commit write, one audit event and a fully ready post-write manifest. The temporary database, file root and listener were removed. Production backfill remains approval-gated and has not run.
