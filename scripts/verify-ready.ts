...
if (!readyUrl || !['discovery', 'commerce'].includes(expectedPhase)) {
  throw new Error('Usage: verify-ready <ready-url> <discovery|commerce>');
}
...
for (let attempt = 1; attempt <= 10; attempt += 1) {
  try {
    ...
    const errors = validateLaunchReadiness(body, expectedPhase as LaunchPhase);
