...
export async function registerCommerceRoutes(app: FastifyInstance) {
  app.get('/cart', async (request, reply) => {
    ...
    return reply.view('pages/catalog/cart.ejs', pageModel(request, { title: 'Cart', description: 'Review your Seedexchange cart.', canonical: null, ...cart, checkoutEnabled: false }));
  });
