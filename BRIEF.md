...
- Local TypeScript, the clean build and 104 Vitest checks pass. One PostgreSQL-backed migration test is skipped when `TEST_DATABASE_URL` is not configured. Chromium public acceptance most recently passed 13 checks with two deliberate duplicate-viewport skips.
- The immutable discovery release manifest verifies 304 allowlisted files, pins migration `003_discovery_migration_scope.sql` and contains no runtime `node_modules`.
