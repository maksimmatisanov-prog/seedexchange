...
if ! sudo systemctl restart "$service" || ! node dist/scripts/verify-ready.js http://127.0.0.1:4200/ready discovery "$expected_migration" || ! node dist/scripts/verify-discovery-runtime.js --origin=http://127.0.0.1:4200 --migration="$expected_migration" --organization="$organization_path" --product="$product_path" --media="$media_path" || ! node dist/scripts/verify-discovery-load.js --origin=http://127.0.0.1:4200 --organization="$organization_path" --product="$product_path" --requests=72 --concurrency=6 --timeout-ms=3000 --p95-ms=750 || ! sudo systemctl start seedexchange-production-sitemap.service || ! sudo systemctl start seedexchange-production-outbox.service || ! sudo -u seedexchange -- node dist/scripts/verify-production-observation.js --migration="$expected_migration" --organization="$organization_path" --product="$product_path" || ! sudo systemctl enable "$service" || ! sudo systemctl enable --now seedexchange-production-sitemap.timer seedexchange-production-outbox.timer || ! sudo systemctl is-active --quiet "$service" seedexchange-production-sitemap.timer seedexchange-production-outbox.timer; then
  if [[ -n "$previous" && "$previous" == "$releases"/* && -d "$previous" ]]; then
    sudo systemctl disable --now seedexchange-production-sitemap.timer seedexchange-production-outbox.timer || true
    ln -sfn "$previous" "$root/current.next"
    mv -Tf "$root/current.next" "$root/current"
    sudo systemctl restart "$service"
    sudo systemctl enable "$service"
    sudo systemctl enable --now seedexchange-production-sitemap.timer seedexchange-production-outbox.timer
